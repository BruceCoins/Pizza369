...
### 【1】拥有 Copy strait 属性的类型：   
```rust   
    可以直接赋值 b = a，要求a、b是如下同类型
...
### 【2】使用 Box 的集合（Heap操作）：   
```rust
    不能直接赋值 b = a 操作，否则 a 的值将不再可用，要求a、b是如下同类型 

    Vec、String、HashMap...
```    
- **Box 内存释放原则**：  
```rust
如果一个变量拥有一个 box ，当 Rust 释放该变量的 stack frame 时，Rust 同时释放该 box 的堆内存（Heap） 
```    
...
```
### 【2】解引用【符号 *】获取数据，引用【符号 &】获取当前变量的指针(地址)    
...
### 【5】可变引用（&mut x）提供对数据 ”唯一的“ 且 ”非拥有的（通过&借用，不转让所有权）“ 访问   
