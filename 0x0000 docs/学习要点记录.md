...
- 【4】库合约的调用
  ...
  - 方法二：使用 ``库合约名.函数名()`` 的方式调用
    ...
    ```
### 13、接收 ETH 时 ``receive()`` 和 ``fallback()`` 函数处理   
...
### 14、发送 ETH 时 ``transfer()``、``send()``、``call()`` 函数处理 
- 【1】-----``transfer()``------
...
定义被调用合约名 OthereContract，包含函数 setX(uint){}、getX(){}、getX2(){}、``payable``修饰的setX2(){}   

- 【1】 ``被调用合约名(被调用合约地址).被调用函数名()``  
```solidity  
    //为其他合约设置 X 值，注意参数类型是 address 类型
    function callSetX(address _Address, uint x){
        OtherContract(_Address).setX(x);
    }
```  
- 【2】 ``将主动调用的函数中 参数类型 address 改为 被调用合约名``  
```solidity
    // 此函数 参数类型 为被调用合约名
...
```
- 【4】``如果被调用合约的函数是 payable 的，可以通过此函数转账``
