- [Truffle 框架智能合约测试](#truffle-框架智能合约测试)
  - [1、Truffle测试概述](#1truffle测试概述)
  - [2、示例项目](#2示例项目)
  - [3、用 Solidity 编写测试用例](#3用-solidity-编写测试用例)
    - [3.1\> 单元测试](#31-单元测试)
    - [3.2\> 集成测试](#32-集成测试)
  - [4、用 JavaScript 编写测试用例](#4用-javascript-编写测试用例)
    - [4.1\> 集成测试](#41-集成测试)
  - [参考资料：](#参考资料)
...
- **智能合约测试的类型主要有两种：**   
  1> 单元测试：【solidity】测试合约的单个函数或功能。  
  2> 集成测试：【solidity】测试合约间的交互，确保各部分代码组合起来也可以按期望的方式运行。这确保了继承和依赖注入等机制按预期运行。               【javaScript】测试从区块链外部调用智能合约，确保调用智能合约时，DApp前端正常工作。
  
- 从测试方法上来说：  
  `使用 Solidity 编写测试用例`是测试智能合约的内部行为。编写单元测试调用合约方法，检测智能合约函数的返回值以及状态变量的值；编写集成测试来检查智能合约直接的交互。  
  `使用 JavaScript 编写测试用例`是确保合约正确的外部行为。主要使用web3.js、 Mocha 和 Chai 断言 编写测试脚本。
