...
- [环境搭建](#环境搭建)
  ...
  - [7、编写合约](#7编写合约)
  - [8、编译部署](#8编译部署)
    - [1\> ganache的使用](#1-ganache的使用)
      - [1.1\> 方法一：使用 ganache-cli](#11-方法一使用-ganache-cli)
      - [1.2\> 方法二：使用 ganache 桌面端](#12-方法二使用-ganache-桌面端)
    - [2\> 只部署在开发环境](#2-只部署在开发环境)
      - [2.1\> 配置文件](#21-配置文件)
      - [2.2\> 编译合约](#22-编译合约)
      - [2.3\> 部署合约](#23-部署合约)
    - [3\> 连接以太坊 测试网、主网](#3-连接以太坊-测试网主网)
      - [3.1\> infura 获取API\_KEY](#31-infura-获取api_key)
      - [3.2\> chainlink faucets 领取测试网 ETH](#32-chainlink-faucets-领取测试网-eth)
      - [3.3\> 配置文件](#33-配置文件)
      - [3.4\> 编译合约](#34-编译合约)
      - [3.5\> 部署合约](#35-部署合约)
...
### 1> ganache的使用  
#### 1.1> 方法一：使用 ganache-cli
直接启动 ganache-cli，然后对合约进行编译、部署即可
```
ganache-cli
``` 
#### 1.2> 方法二：使用 ganache 桌面端
...
#### 3.2> chainlink faucets 领取测试网 ETH  
